	 ...
	 _initt:function(){
	  ...
	  _addThWidth:function($th,width){
			...
			if(!this.cwdiths){
				this.cwdiths=[];
				this._$theadTable.find(".cindex>th").each(function(){
					self.cwdiths.push($(this).width());
				});
			}
			if(!this._tableWidth){
				this._tableWidth = this._$theadTable.width();
			}
			this._tableWidth += width;
			var tableWidth  =  this._tableWidth;
			var data 		= $th.data(this._th_data_name);
			var toWidth	= [];
			...
			var $indexs = this._$theadTable.find(".cindex>th");
			
			for(var i=0;i<data.thc.length;i++){
					var ind = $indexs.index(data.thc[i]);
					self.cwdiths[ind] = self.cwdiths[ind]+width/data.thc.length;
			}
			
			var $bindexs = this._$tbodyTable.find(".cindex>th");
			
			for(var i=0;i<this.cwdiths.length;i++){
				this._$theadTable[0].rows[0].cells[i].style.width=self.cwdiths[i]+"px";
				this._$tbodyTable[0].rows[0].childNodes[i].style.width=self.cwdiths[i]+"px";
			}
